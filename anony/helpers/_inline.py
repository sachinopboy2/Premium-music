...
class Inline:
    ...
    def help_markup(self, _lang: dict, back: bool = False):
        if back:
            rows = [
                [
                    self.ikb(text=f"🔙 {_lang['back']}", callback_data="help back", style=ButtonStyle.PRIMARY),
            ...
            buttons = [
                self.ikb(text=f"✨ {_lang[f'help_{i}']}", callback_data=f"help {cb}", style=ButtonStyle.PRIMARY)
    ...
    def lang_markup(self, _lang: str):
        ...
        buttons = [
            self.ikb(
                text=f"🌐 {name} {'✔️' if code == _lang else ''}",
    ...
    def queue_markup(self, chat_id: int, _text: str, playing: bool):
        ...
        return self.ikm(
            [[self.ikb(text=f"🎵 {_text}", callback_data=f"controls {action} {chat_id} q", style=ButtonStyle.PRIMARY)]]
        )
    ...
    def settings_markup(self, lang: dict, admin_only: bool, cmd_delete: bool, language: str, chat_id: int):
        return self.ikm(
            [
                [
                    self.ikb(f"🎭 {lang['play_mode']} ➜", callback_data="settings", style=ButtonStyle.DEFAULT),
                    self.ikb(text=f"✅ {admin_only}", callback_data="settings play", style=ButtonStyle.SUCCESS),
                ],
                [
                    self.ikb(f"🗑️ {lang['cmd_delete']} ➜", callback_data="settings", style=ButtonStyle.DEFAULT),
                    self.ikb(text=f"🔘 {cmd_delete}", callback_data="settings delete", style=ButtonStyle.PRIMARY),
                ],
                [
    ...
    def start_key(self, lang: dict, private: bool = False):
        rows = [
            ...
            [
                ...
                self.ikb(text="💘ᴅᴇᴠʟᴏᴘᴇʀ", url=config.OWNER_ID", style=ButtonStyle.PRIMARY),
            ],
            [
                self.ikb(text="💎 sᴜᴘᴘᴏʀᴛ", url=config.SUPPORT_CHAT, style=ButtonStyle.PRIMARY),
                self.ikb(text="📢 ᴄʜᴀɴɴᴇʟ", url=config.SUPPORT_CHANNEL, style=ButtonStyle.SUCCESS),
            ],
        ]
    ...
    def yt_key(self, link: str):
        return self.ikm(
            [
                [
                    self.ikb(text="📋 ᴄᴏᴘʏ", copy_text=link, style=ButtonStyle.PRIMARY),
                    self.ikb(text="📺 ʏᴏᴜᴛᴜʙᴇ", url=link, style=ButtonStyle.DANGER),
                ]
            ]
        )
